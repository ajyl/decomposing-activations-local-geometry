...
class WeightedReservoirSampler:
    def __init__(
        ...
        self.m = m
        self.query_or_key = query_or_key
        assert self.query_or_key is not None
        assert self.query_or_key in ["query", "key"]
        self.w = (
            weights.to(device)
    ...
    def sample(self, loader: DataLoader) -> torch.Tensor:
        ...
        for _q, _k in loader:
            ...
            if pool is None:
                ...
                keys = torch.full(
                    (self.m,), -float("inf"), device=self.device, dtype=torch.float32
                )
...
@torch.no_grad()
def make_orthonormal_projector(
    D: int, d: int, device=None, dtype=torch.float32, seed: Optional[int] = None
) -> torch.Tensor:
    if d > D:
...
class KMeansTorch:
    ...
    def _assign_streamed(self, X: torch.Tensor, C: torch.Tensor):
        ...
        for s in range(0, N, self.block_x):
            ...
            if self.metric == "cosine":
                best_val = torch.full(
                    (bx,), -float("inf"), device=self.device, dtype=self.dtype
                )
            else:
                best_val = torch.full(
                    (bx,), float("inf"), device=self.device, dtype=X.dtype
                )
    ...
    def _kpp_streamed(self, X: torch.Tensor) -> torch.Tensor:
        N = X.size(0)
        X = X.to(
            self.device, dtype=self.dtype if self.metric == "cosine" else torch.float32
        )
        if self.metric == "cosine":
            X = _normed(X)
        ...
        if self.metric == "cosine":
            ...
            def dblock(xb, cb):
                return 1.0 - (xb @ cb.T).to(torch.float32).squeeze(1).clamp(-1, 1)

        else:

            def dblock(xb, cb):
                return _pairwise_dist2_euclidean(xb, cb).squeeze(1)
    ...
    def fit(self, X: torch.Tensor) -> torch.Tensor:
        ...
        for r in range(self.restarts):
            ...
            for i in range(self.n_iter):
                ...
                nonzero = counts > 0
                newC = C.clone()
                newC[nonzero] = sums[nonzero] / counts[nonzero].unsqueeze(1).to(
                    sums.dtype
                )
                if self.metric == "cosine":
...
@torch.no_grad()
def _assign_projected_streamed(
    x_proj: torch.Tensor, C_proj: torch.Tensor, metric: str, block_c: int = 8192
):
    B = x_proj.size(0)
    k = C_proj.size(0)
    ...
    for _ in range(max_epochs):
        ...
        for _q, _k in loader:
            if query_or_key == "query":
                ...
                raise ValueError("query_or_key must be 'query' or 'key'")
...
class ReservoirKMeans:
    def __init__(
        ...
        *,
        ...
        self.query_or_key = query_or_key
        assert query_or_key in ["query", "key"]
        ...
        for _ in range(max_epochs):
            ...
            for _q, _k in loader:
                ...
                if metric == "cosine":
                    ...
                    best = torch.full(
                        (xb.size(0),), -float("inf"), device=device, dtype=dtype
                    )
                else:
                    best = torch.full(
                        (xb.size(0),), float("inf"), device=device, dtype=torch.float32
                    )
                best_idx = torch.full(
                    (xb.size(0),), -1, device=device, dtype=torch.long
                )
    ...
    def fit(
        ...
        sampler = WeightedReservoirSampler(
            self.pool_size,
            query_or_key=self.query_or_key,
            weights=weights,
            device=self.device,
        )
        ...
        if use_proj:
            d = int(self.proj_dim)
            self.R = make_orthonormal_projector(
                D, d, device=self.device, dtype=self.proj_dtype, seed=self.seed
            )
